...
bool contains_str (const string &needle, ...)
{
  char *arg;
  va_list va;
  bool res = false;
...
/*
pair<printable *, printable *> eval (printable *sql, int num=1)
{
  string decl = "\tcursor_handle" + num + " integer;\n" +
    "\tfeedback"+num+" integer;\n";

  string body = "\tcursor_handle"+num+" := dbms_sql.open_cursor;\n" +
    "\tdbms_sql.parse (cursor_handle"+num+", " + oracle_escape (sql->str ()) + ", dbms_sql.native);\n" +
    "\tfeedback"+num+" := dbms_sql.execute (cursor_handle);\n" +
    "\tdbms_sql.close_cursor (cursor_handle"+num+");\n";

  return pair<printable *, printable *> (new text (decl), new text (body));
}
*/
